...
def test_native_grpc_sidecar_is_disabled_by_default() -> None:
    backend = SGLangProtocol()

    assert backend.native_grpc_sidecar is False
    assert backend.native_grpc_port == 50051
    assert backend.sidecar_args == [
        "--sglang-connections",
        "8",
...
def test_prefill_leader_launches_native_grpc_and_sidecar() -> None:
    ...
    assert "--disaggregation-bootstrap-port 30001" in script
...
def test_invalid_native_grpc_port_is_rejected() -> None:
    ...
    with patch("srtctl.core.slurm.get_hostname_ip", return_value="10.0.0.1"):
        try:
            ...
            assert "native_grpc_port must be between 1 and 65535" in str(error)
        else:
            raise AssertionError("expected an invalid native gRPC port to fail")
